...
def main():
    ...
    category_at_zero = len(negative_buckets) - 1
    ...
    train_inputs = tf.convert_to_tensor(train, dtype=tf.int32)
    # train_labels = tf.convert_to_tensor(train[2:], dtype=tf.int32)

    validation_inputs = tf.convert_to_tensor(validation, dtype=tf.int32)
    # validation_labels = tf.convert_to_tensor(validation[2:], dtype=tf.int32)

    test_inputs = tf.convert_to_tensor(test[1: len(test) - 1], dtype=tf.int32)
    test_labels = tf.convert_to_tensor(test[2:], dtype=tf.int32)
    ...
    # Now use fit
    history = model.fit(x=train_inputs, y=y_train, batch_size=32, epochs=15, validation_data=(val_inputs, y_val),
